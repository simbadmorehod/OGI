...
BATCH_SIZE = 512  # Оптимальный размер батча для CPU
...
def update_all_embeddings(retries: int = 3):
    db = next(get_db())
    embedder = BGEEmbedder(device="cpu")  # Используем CPU для стабильности
        ...
        with tqdm(total=total_to_process, desc="Обновление эмбеддингов") as pbar:
            ...
            while True:
                try:
                    ...
                    # Генерация эмбеддингов с повторными попытками
                    embeddings = []
